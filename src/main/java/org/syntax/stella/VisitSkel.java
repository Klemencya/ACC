...
public class VisitSkel {
    ...
    private final ProgramVisitor<Program, Map> programVisitor = new ProgramVisitor<>();

    public ProgramVisitor<Program, Map> getProgramVisitor() {
    ...
    public class ProgramVisitor<R, A> implements org.syntax.stella.Absyn.Program.Visitor<R, A> {
        public R visit(org.syntax.stella.Absyn.AProgram p, A arg) { /* Code for AProgram goes here */
            ...
            for (org.syntax.stella.Absyn.Decl x : p.listdecl_) {
                DeclFun declFun = (DeclFun) x;
                R functionReturnType = x.accept(new DeclVisitor<R, A>(), arg);
                ((HashMap) arg).put(declFun.stellaident_, functionReturnType);
            }
            return null;
        }
    }
    ...
    public class DeclVisitor<R, A> implements org.syntax.stella.Absyn.Decl.Visitor<R, A> {
        public R visit(org.syntax.stella.Absyn.DeclFun p, A arg) { /* Code for DeclFun goes here */
            ...
            System.out.println("Идем в функцию: " + p.stellaident_);
//            System.out.println(p.stellaident_);
            for (org.syntax.stella.Absyn.ParamDecl x : p.listparamdecl_) {
                AParamDecl paramDecl = (AParamDecl) x;
                paramDecl.accept(new ParamDeclVisitor<R, A>(), arg);
                ((HashMap) arg).put(paramDecl.stellaident_, paramDecl.type_.getClass());
            } //TODO чистить стек после выхода из функции
            R expectedReturnType = p.returntype_.accept(new ReturnTypeVisitor<>(), arg);
            p.throwtype_.accept(new ThrowTypeVisitor<R, A>(), arg);
            for (org.syntax.stella.Absyn.Decl x : p.listdecl_) {
                x.accept(new DeclVisitor<R, A>(), arg);
            }
            R actualReturnType = p.expr_.accept(new ExprVisitor<>(), arg);
            if (actualReturnType != expectedReturnType) {
                throw new RuntimeException("Expected return type %s doesn't match actual return type %s"
                        .formatted(expectedReturnType, actualReturnType)
                );
            }
...
//            p.stellaident_
    ...
    public class ReturnTypeVisitor<R, A> implements ReturnType.Visitor<R, A> {
        public R visit(org.syntax.stella.Absyn.NoReturnType p, A arg) {
            return (R) NoReturnType.class;
        }

        public R visit(org.syntax.stella.Absyn.SomeReturnType p, A arg) {
//            p.type_.accept(new TypeVisitor<R, A>(), arg);
            return (R) p.type_.getClass(); //TODO поумнее?
        }
    }
    ...
    public class ExprVisitor<R, A> implements org.syntax.stella.Absyn.Expr.Visitor<R, A> {
        public R visit(org.syntax.stella.Absyn.If p, A arg) {
            R expr1Type = p.expr_1.accept(new ExprVisitor<R, A>(), arg);
            if (expr1Type != TypeBool.class) {
                throw new RuntimeException("If condition type %s is not TypeBool".formatted(expr1Type));
            }
            R expr2Type = p.expr_2.accept(new ExprVisitor<R, A>(), arg);
            R expr3Type = p.expr_3.accept(new ExprVisitor<R, A>(), arg);
            if (expr2Type != expr3Type) {
                throw new RuntimeException("If branches return different types: %s %s".formatted(expr2Type, expr3Type));
            }
            return expr2Type;
        }
        ...
        public R visit(org.syntax.stella.Absyn.Abstraction p, A arg) { /* Code for Abstraction goes here */
            for (org.syntax.stella.Absyn.ParamDecl x : p.listparamdecl_) {
                AParamDecl paramDecl = (AParamDecl) x;
                R type = paramDecl.accept(new ParamDeclVisitor<R, A>(), arg);
                ((HashMap) arg).put(paramDecl.stellaident_, type);
            }
            p.expr_.accept(new ExprVisitor<R, A>(), arg);
            return (R) TypeFun.class;
        }//((HashMap)arg).put((AParamDecl))
        ...
        public R visit(org.syntax.stella.Absyn.Application p, A arg) { /* Code for Application goes here */
            R functionWithReturnType = p.expr_.accept(new ExprVisitor<R, A>(), arg);
            for (org.syntax.stella.Absyn.Expr x : p.listexpr_) { //TODO проверять, что параметр(s) правильного типа
                x.accept(new ExprVisitor<R, A>(), arg);
            }
            return (R) ((HashMap) arg).get(((Var) p.expr_).stellaident_); //TODO неправильно точно
        }
        ...
        public R visit(org.syntax.stella.Absyn.Var p, A arg) { /* Code for Var goes here */
            if (arg instanceof Map) {
                return ((Map<String, R>) arg).get(p.stellaident_);
            }
            return null;
        }
    }
